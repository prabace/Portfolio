...
export const Work = () => {
  ...
  return (
    <div className="h-screen fixed  py-40 text-white mt w-[100%] bg-black">
      <div className="grid mobile:grid-cols-1 tablet:grid-cols-3 ">
        <div className="mobile:hidden tablet:flex col-span-2">
          <div className={reveal === "rentedWheels" ? "flex h-screen w-[100%]" : "hidden"}>
            <img alt="rented" src={rented} className="rounded-2xl tablet:max-w-tablet laptop:max-w-laptop desktop:max-w-desktop"/>
          </div>
          <div className={reveal === "projectManager" ? "flex h-screen w-[100%]" : "hidden"}>
            <img alt="project"  src={project} className="rounded-2xl tablet:max-w-tablet laptop:max-w-laptop desktop:max-w-desktop object-cover"/>
          </div>
          
        </div>
        <div className="ml-4">
          <h2 className="text-4xl font-ham ">WORKS</h2>
          <hr className="my-2 w-[600px]"/>

          <div className="flex flex-col mt-10 font-heading text-2xl">
            <ul className="cursor-pointer">
                <div>
              <li className="p-4 text-3xl" onMouseEnter={()=>handleHover('rentedWheels') } onMouseLeave={()=>handleHover(false)}><a href="rentedwheels">{reveal === "rentedWheels" ?<ChevronRightIcon className="forward mr-6" ></ChevronRightIcon>:""}Rented Wheels</a></li>
              </div>
              <hr className="my-2 w-[600px]"/>
              {/* <div>
